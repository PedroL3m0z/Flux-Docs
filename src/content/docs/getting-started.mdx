...
Two ways to boot the stack — pick one:

<Tabs>
  <TabItem label="All-in-one image">

The fastest path — API + PostgreSQL + Redis in a single published container,
nothing to clone or wire up:
...
Built for single-host / self-hosted setups, not horizontal scaling (a restart
cycles every process).

  </TabItem>
  <TabItem label="Docker Compose">

Clone the repo and run the API, Postgres and Redis as separate services — the
choice for scalable deployments:
